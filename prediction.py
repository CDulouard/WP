...
def main():
    data = pd.read_csv(WORK_FILE, sep=',', names=COLUMNS)  # importing data
    ...
    features = encoded_data.loc[:, encoded_data.columns[0:-1]]
    targets = encoded_data.loc[:, encoded_data.columns[-1]]
    ...
    normalized_features = pd.DataFrame(scaler.fit_transform(features.values))
    ...
    sdg_model = SGDClassifier(loss="hinge", penalty="l2", max_iter=1000000)
    ...
    tree_model = tree_model.fit(features_train, targets_train)
    print("\n\nTree :\nTest set score: {:.2f}%".format(
